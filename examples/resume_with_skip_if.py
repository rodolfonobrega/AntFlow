"""
Smart resume example using skip_if conditions.

Demonstrates how to use the skip_if parameter for automatic stage skipping:
- Items carry their own state (e.g., dataclass with boolean flags)
- skip_if lambda checks if work was already done
- Re-running the same items automatically skips completed stages

This is simpler than checkpoint-based resume when:
- Items are mutable and carry their own state
- You can safely re-submit the same objects
- You don't need to track progress externally
"""
...
async def main():
    ...
    for job in jobs:
        status = []
        if job.dataset_uploaded:
            status.append("Uploaded")
        if job.model_trained:
            status.append("Trained")
        if job.results_saved:
            status.append("Saved")
    ...
    for job in results_2:
        val = job.value
        status = []
        if val.dataset_uploaded:
            status.append("Uploaded")
        if val.model_trained:
            status.append("Trained")
        if val.results_saved:
            status.append("Saved")
